app.controller("BookDetailCtrl", function ($scope, $http, httpService, $stateParams, $ionicLoading, $ionicPopup, $ionicPlatform, $cordovaClipboard, $cordovaToast, $timeout, $rootScope) {
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        $scope.content_data = {
            ...
            writer: getPostMetaValueById($scope.data.post.post_meta, 'writer'),
            publisher: getPostMetaValueById($scope.data.post.post_meta, 'publisher'),
        ...
        if(isPhoneGap())
        {
            ...
            savePostJSONToDB($stateParams.id, $scope.data.post.term_id, 'BookDetailCtrl', $scope.data);
            //console.log('saved');
        }
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('httpService:getHighResDataSuccess', function (event, args) {
        ...
        $scope.content_data = {
            ...
            writer: getPostMetaValueById(updated_data.post.post_meta, 'writer'),
            publisher: getPostMetaValueById(updated_data.post.post_meta, 'publisher'),
        ...
        {
            //console.log($stateParams.id + " " + $scope.data.post.term_id + " " + 'ArticleDetailCtrl' + " " + $scope.data);
            savePostJSONToDB($stateParams.id, updated_data.post.term_id, 'BookDetailCtrl', updated_data);
            //console.log('saved');
        }
