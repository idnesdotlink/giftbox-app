app.controller("DefaultCtrl", function ($scope, $http, httpService, $stateParams, $ionicPlatform, $rootScope) {
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'DefaultCtrl', $scope.data);
        }
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('httpService:getHighResDataSuccess', function (event, args) {
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'DefaultCtrl', updated_data);
        }
