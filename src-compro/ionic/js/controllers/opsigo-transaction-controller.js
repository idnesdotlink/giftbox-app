app.controller("OpsigoTransactionListCtrl", function ($scope, $http, httpService, $stateParams, $window, $ionicPlatform) {
    ...
    $scope.$on('httpService:postTokenSuccess', function () {
        ...
        //console.log(token);
        var url = user_opsigo_transaction_list + user_id;
        httpService.get($scope, $http, url, 'booking-transaction', token);
        console.log('OpsigoTransactionListCtrl');
    });
    ...
    $scope.$on('httpService:getBookingTransactionSuccess', function () {
        ...
        $scope.content_data = {
            ...
            title: 'Booking Transaction History'
        };

        console.log("Mendapat booking transaksi");
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB('-1'+user_id, 'OpsigoTransactionListCtrl', $scope.data);
            ...
            for(var i=0; i< $scope.data.term_posts.length; i++)
            {
                ...
                savePostJSONToDB($scope.data.term_posts[i].id, $scope.data.term_posts[i].term_id, 'BookingTransactionListCtrl', $scope.data.term_posts[i]);
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('OpsigoTransactionListCtrl:getOfflineDataSuccess', function () {
        $scope.content_data = {
            transactions: $scope.data.transactions,
            title: 'Booking Transaction History'
        };
    ...
    $scope.$on('httpService:postTokenGetMoreDataSuccess', function () {
        token = $scope.data.token;
        //console.log(token);
        var url = user_opsigo_transaction_list + 274;
