app.controller("ProfileCtrl", function ($scope, $http, httpService, $stateParams, $ionicPlatform, $rootScope) {
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        ...
        $scope.content_data = {
            ...
            name: getPostMetaValueById($scope.data.post.post_meta, 'name').value,
            biography: $scope.data.post.content,
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'ProfileCtrl', $scope.data);
        }
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('httpService:getHighResDataSuccess', function (event, args) {
        ...
        $scope.content_data = {
            ...
            name: getPostMetaValueById(updated_data.post.post_meta, 'name').value,
            biography: updated_data.post.content,
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'ProfileCtrl', updated_data);
        }
