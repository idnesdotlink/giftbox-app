app.controller("ProductCtrl", function ($scope, $http, httpService, $stateParams, $window, $ionicPlatform, $ionicLoading) {
    ...
    $scope.$on('httpService:postTokenSuccess', function () {
        ...
        httpService.get($scope, $http, url, 'content', token, 0,
            encodeURIComponent($scope.input.orderBySelected), undefined , encodeURI($scope.product_search.search_text));
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('ProductCtrl:getOfflineDataSuccess', function () {
        scope.content_data = {
            title: $scope.data.term.title,
            products: $scope.data.term_posts,
            term_meta: $scope.data.term_meta
