app.controller("AirlineBookingSearchReturnCtrl", function ($scope,
                                     $rootScope,
                                     $http,
                                     httpService,
                                     $stateParams,
    ...
    if($stateParams.prefered_airline_id !== undefined){
        $scope.prefered_airline_id = $stateParams.prefered_airline_id;
        $scope.search_airline.push($scope.prefered_airline_id);
    ...
    $ionicPlatform.ready(function () {
        console.log('Airline Booking Search Return Controller');
        airlineFindReturnFlights();
    });

    function airlineFindReturnFlights() {
        console.log('airlineFindReturnFlights');
        var url = opsigo_request_flight_v3;
        var obj = serializeData({
            origin: $scope.airline.search_destination_code,
            destination: $scope.airline.search_origin_code,
            departdate: formatDate($scope.airline.return_date),
    ...
    $scope.$on('httpService:postOpsigoRequestFlightV3Success', function () {
        ...
        $scope.content_data = {
            airlines: $scope.data,
            return_flights: $scope.data.Schedules[0].Flights
        };

        $rootScope.airline_search_return_result = $scope.content_data.return_flights;
        console.log($scope.content_data.return_flights);
