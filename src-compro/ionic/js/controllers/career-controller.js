app.controller("CareerCtrl", function ($scope, $http, httpService, $stateParams, $ionicSlideBoxDelegate, $ionicPlatform, $cordovaClipboard, $cordovaToast, $timeout) {
    ...
    // if get token success, request data career
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'CareerCtrl', $scope.data);
            ...
            for(var i=0; i< $scope.data.term_posts.length; i++)
            {
                ...
                savePostJSONToDB($scope.data.term_posts[i].id, $scope.data.term_posts[i].term_id, 'CareerCtrl', $scope.data.term_posts[i]);
    ...
    // if get career data failed, request token again
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('httpService:getHighResDataSuccess', function (event, args) {
        ...
        {
            $scope.content_data.careers[((updated_data.next_page-2)*6)+i].featured_image_path = updated_data.term_posts[i].featured_image_path;
            $scope.content_data.careers[((updated_data.next_page-2)*6)+i].content = updated_data.term_posts[i].content;
        }

        // save to db first page
        if(isPhoneGap() && updated_data.next_page==2)
        {
            saveTermJSONToDB($stateParams.id, 'CareerCtrl', updated_data);
            //console.log($scope.data.term_posts);
            clearPostDataByTermId($stateParams.id);
            for(var i=0; i< updated_data.term_posts.length; i++)
            {
                //console.log($scope.data.term_posts[i]);
                savePostJSONToDB(updated_data.term_posts[i].id, updated_data.term_posts[i].term_id, 'CareerCtrl', updated_data.term_posts[i]);
    ...
    {
        var img_src = $scope.content_data.careers[index].featured_image_path;
        //console.log('share');
        if(isPhoneGap())
        {
            if(isAndroid())
    ...
    $scope.slideHasChanged = function($index){
        if($index+2 >= $scope.content_data.careers.length)
        {
            $scope.loadMoreData();
        }
    };
    ...
    $scope.$on('httpService:getMoreDataSuccess', function () {
        ...
        Array.prototype.push.apply($scope.content_data.careers, $scope.data.term_posts);
