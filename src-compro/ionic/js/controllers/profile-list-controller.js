app.controller("ProfileListCtrl", function ($scope, $http, httpService, $stateParams, $ionicPlatform) {
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        $scope.content_data = {
            ...
            profiles: $scope.data.term_posts
        };
        $scope = replaceLanguageTitle($scope);

        $scope.content_data.profiles = changeFontSize($scope.content_data.profiles);
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'ProfileListCtrl', $scope.data);
            ...
            for(var i=0; i< $scope.data.term_posts.length; i++)
            {
                ...
                savePostJSONToDB($scope.data.term_posts[i].id, $scope.data.term_posts[i].term_id, 'ProfileListCtrl', $scope.data.term_posts[i]);
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('httpService:getHighResDataSuccess', function (event, args) {
        ...
        {
            $scope.content_data.profiles[((updated_data.next_page-2)*6)+i].featured_image_path = updated_data.term_posts[i].featured_image_path;
//            $scope.content_data.profiles[((updated_data.next_page-2)*6)+i].content = updated_data.term_posts[i].content;
        }

        // save to db first page
        if(isPhoneGap() && updated_data.next_page==2)
        {
            saveTermJSONToDB($stateParams.id, 'ProfileListCtrl', updated_data);
            //console.log($scope.data.term_posts);
            clearPostDataByTermId($stateParams.id);
            for(var i=0; i< updated_data.term_posts.length; i++)
            {
                //console.log($scope.data.term_posts[i]);
                savePostJSONToDB(updated_data.term_posts[i].id, updated_data.term_posts[i].term_id, 'ProfileListCtrl', updated_data.term_posts[i]);
    ...
    $scope.$on('httpService:getMoreDataSuccess', function () {
        ...
        Array.prototype.push.apply($scope.content_data.profiles, $scope.data.term_posts);
        ...
        //console.log(($scope.content_data.profiles).length);
        $scope.$broadcast('scroll.infiniteScrollComplete');
