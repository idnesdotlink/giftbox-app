app.controller("OnlineReservationListCtrl", function ($scope, $http, httpService, $stateParams, $window, $ionicPlatform) {
    ...
    $scope.$on('httpService:postTokenSuccess', function () {
        ...
        console.log('OnlineReservationListCtrl');
    });
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        $scope.content_data = {
            ...
            reservations: $scope.data.term_posts
        };
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB($stateParams.id, 'OnlineReservationListCtrl', updated_data);
            ...
            for(var i=0; i< $scope.data.term_posts.length; i++)
            {
                ...
                savePostJSONToDB($scope.data.term_posts[i].id, $scope.data.term_posts[i].term_id, 'OnlineReservationListCtrl', $scope.data.term_posts[i]);
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('httpService:getHighResDataSuccess', function (event, args) {
        ...
        {
            $scope.content_data.reservations[((updated_data.next_page-2)*6)+i].featured_image_path = updated_data.term_posts[i].featured_image_path;
            $scope.content_data.reservations[((updated_data.next_page-2)*6)+i].content = updated_data.term_posts[i].content;
        }
        ...
        if(isPhoneGap() && updated_data.next_page==2)
        {
            ...
            {
                //console.log($scope.data.term_posts[i]);
                savePostJSONToDB(updated_data.term_posts[i].id, updated_data.term_posts[i].term_id, 'OnlineReservationListCtrl', updated_data.term_posts[i]);
    ...
    $scope.$on('httpService:getMoreDataSuccess', function () {
        ...
        Array.prototype.push.apply($scope.content_data.reservations, $scope.data.term_posts);
