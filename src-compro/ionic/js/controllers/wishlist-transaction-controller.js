app.controller("WishlistTransactionCtrl", function ($scope, $http, httpService, $stateParams, $window, $ionicPlatform) {
    ...
    var loadCustomText = function() {
      ...
      $scope.wishlist_transaction_history = getMenuText(default_menu_titles.wishlist_transaction_history, "Wishlist Transaction History");
    };
    ...
    $scope.$on('httpService:postTokenSuccess', function () {
        ...
        //console.log(token);
        var url = user_wishlist_transaction + user_id;
        httpService.get($scope, $http, url, 'content', token);
        console.log('WishlistTransactionCtrl');
    });
    ...
    $scope.$on('httpService:getRequestSuccess', function () {
        ...
        $scope.content_data = {
            wishlist_transactions: $scope.data.wishlist_transactions,
            title: $scope.wishlist_transaction_history
        };

        console.log($scope.content_data.wishlist_transactions);
        ...
        if(isPhoneGap())
        {
            saveTermJSONToDB('-1'+user_id, 'WishlistTransactionCtrl', $scope.data);
            ...
            for(var i=0; i< $scope.data.term_posts.length; i++)
            {
                ...
                savePostJSONToDB($scope.data.term_posts[i].id, $scope.data.term_posts[i].term_id, 'WishlistTransactionCtrl', $scope.data.term_posts[i]);
...
//        httpService.post_token($scope, $http, url, obj, 'content');
    ...
    $scope.$on('WishlistTransactionCtrl:getOfflineDataSuccess', function () {
        $scope.content_data = {
            wishlist_transactions: $scope.data.wishlist_transactions,
            title: $scope.wishlist_transaction_history
        };
    ...
    $scope.$on('httpService:getMoreDataSuccess', function () {
        ...
        Array.prototype.push.apply($scope.content_data.wishlist_transactions, $scope.data.wishlist_transactions);
